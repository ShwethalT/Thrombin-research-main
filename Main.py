...
def pIC50(input):
    """
    Converts IC50 values from nm to M and applies log 10 to it.

    Parameters:
    input (DataFrame): Input dataframe with 'standard_value_norm' column.

    Returns:
    DataFrame: Transformed dataframe with 'pIC50' column.
    """
    pIC50 = []
...
def lipinski(smiles, verbose=False):
    ...
    Parameters:
    smiles (Series): Series containing SMILES representations.
    verbose (bool, optional): If True, print additional information. Defaults to False.

    Returns:
    DataFrame: Lipinski descriptors for the molecules.
    """
...
def norm_value(input):
    """
    Values greater than 100,000,000 fixed at 100,000,000 otherwise the negative logarithmic value will become negative

    Parameters:
    input (DataFrame): Input dataframe with 'standard_value' column.

    Returns:
    DataFrame: Transformed dataframe with 'standard_value_norm' column.
    """
...
def updated_bioactivity_class(df, pIC50_column="pIC50"):
    ...
    Parameters:
    df (DataFrame): Input dataframe with pIC50 values.
    pIC50_column (str, optional): Column name for pIC50 values. Defaults to 'pIC50'.

    Returns:
    DataFrame: DataFrame with added 'class' column.
    """
    bioactivity_class = []
...
def mannwhitney(descriptor, verbose=False):
    """
    Performs Mann-Whitney U test on active and inactive classes.

    Parameters:
    descriptor (str): Descriptor for the test.
    verbose (bool, optional): If True, print additional information. Defaults to False.

    Returns:
    DataFrame: Results of the Mann-Whitney U test.
    """
...
def main():
    """
    Function created computes a customized thrombin target dataset from the ChEMBL database. 
    Drops null values, duplicate values and unnecessary columns. Assigns bioactivity classes : active, inactive and intermediate, 
    based on IC50 values. Builds Regression model using Random Forest.
    Trains and tests the model on the dataset and generates performance metrics with LazyRegressor.

    Plots created: 
    1. Std_value before transformation
    2. Frequency Plot of Bioactivity Classes
    3. LogP vs Molecular Weight of compounds
    4. Relationship between each activity level and pIC50
    5. Relationship between each activity level and MW
    6. Relationship between each activity level and LogP
    7. Relationship between each activity level and HDonors
    8. Relationship between each activity level and HAcceptor
    9. Experimental vs Predicted pIC50 Values
    10. Model Comparisons by R-Squared
    11. Model Comparisons by RMSE
    12. Model Comparisons by Time taken
    ...
    result_search = activity.filter(target_chembl_id=thrombin_target).filter(
        standard_type="IC50"
    )
    ...
    print(
        "Total rows of null values in standard_value column of df",
        df.standard_value.isnull().sum(),
    )
    # 106 rows
    print(
        "Total rows of null values in canonical_smiles column of df",
    ...
    df2_nonull.to_csv(
        "fin_thrombin_01_bioactivity_data_raw_na_values_removed.csv", index=False
    )
    ...
    df_combined_cleaned.to_csv("df_combined.csv", index=False)
    # df_combine rows = ['molecule_chembl_id', 'canonical_smiles',
    ...
    plt.title(
        "relationship between each activity level and pIC50",
        fontsize=14,
        fontweight="bold",
    )
    plt.xlabel("Bioactivity class", fontsize=14, fontweight="bold")
    ...
    plt.title(
        "relationship between each activity level and MW",
        fontsize=14,
        fontweight="bold",
    )
    plt.xlabel("Bioactivity class", fontsize=14, fontweight="bold")
    ...
    plt.title(
        "relationship between each activity level and LogP",
        fontsize=14,
        fontweight="bold",
    )
    plt.xlabel("Bioactivity class", fontsize=14, fontweight="bold")
    ...
    plt.title(
        "relationship between each activity level and HDonors",
        fontsize=14,
        fontweight="bold",
    )
    plt.xlabel("Bioactivity class", fontsize=14, fontweight="bold")
    ...
    plt.title(
        "relationship between each activity level and HAcceptor",
        fontsize=14,
        fontweight="bold",
    )
    plt.xlabel("Bioactivity class", fontsize=14, fontweight="bold")
    ...
    # Train Regressor on train and test data
